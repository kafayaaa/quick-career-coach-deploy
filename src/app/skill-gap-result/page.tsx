...
export default function SkillGapResult() {
  ...
  console.log("Skill Gap From Context:", skillGap);
  console.log("Type of: " + typeof skillGap);
  ...
  console.log(
    "LOCAL STORAGE skillGap:",
    JSON.parse(localStorage.getItem("skillGap")!)
  );
          ...
          {skillGap.recommendations.map(
            (rec: SkillRecommendation, i: number) => (
              <div key={i} className="border p-4 rounded-lg mt-4">
                ...
                {/* Recommended Courses */}
                {Array.isArray(rec.recommended_courses) && (
                  <div className="mt-3">
                    <h4 className="font-semibold">Recommended Courses</h4>
                    <ul className="list-disc ml-6">
                      {rec.recommended_courses.map(
                ...
                {/* Practice Projects */}
                {Array.isArray(rec.practice_projects) && (
                  <div className="mt-3">
                    <h4 className="font-semibold">Practice Projects</h4>
                    <ul className="list-disc ml-6">
                      {rec.practice_projects.map(
                ...
                <p className="mt-3 text-sm text-gray-600">
                  Estimated time to learn: {rec.estimated_time_to_learn}
                </p>
              </div>
            )
          )}
